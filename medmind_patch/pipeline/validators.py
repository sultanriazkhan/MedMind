...
def normalize_value_for_validation(value, test_name: str, unit: Optional[str] = None):
    """
    Convert common CBC display units into the ontology scale used by min_value/max_value.

    Your ontology stores WBC and Platelets plausible ranges as absolute cells/uL,
    while many reports display them as 10^3/uL or thousand/uL.
    Example: WBC 7.4 10^3/uL = 7400 /uL.
    RBC ontology already uses the common million/uL scale, so RBC is not multiplied.
    """
    if not _is_numeric(value):
        return value

    numeric = float(value)
    meta = get_test_metadata(test_name)
    canonical = (meta or {}).get("canonical_name", test_name).lower()
    u = (unit or "").strip().lower()

    thousand_units = {"10^3/ul", "10^3/ul", "x10^3/ul", "k/ul", "thou/ul", "10^3/µl", "x10^3/µl", "k/µl", "thousand/ul", "thousand/µl"}

    if canonical in {"wbc", "platelets"} and u in thousand_units:
        return numeric * 1000
...
def validate_value(
    ...
    numeric = normalize_value_for_validation(value, test_name, unit)
